...
| `outcomes/` | baseline-vs-skill outcome protocols + per-skill verdicts | `handsfree` protocol authored; `concierge` **executed** (MIXED); `hotseat` **executed** (PROVEN LIFT); `spelunk` + `scout` + `distill` + `masterplan` + `pilot` + `backend` + `blueprint` + `headroom` + `polish` + `friction` + `ditto` + `proof` + `roadtest` + `sleuth` + `referee` + `hotpath` + `harden` + `unslop` + `janitor` + `frontpage` + `findable` + `frugal` + `cleared` + `runway` + `recall` protocols + graders authored, zero trials (zero-spend, no Codex) — all 28 skills covered |

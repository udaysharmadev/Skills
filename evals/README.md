...
| `outcomes/` | baseline-vs-skill outcome protocols + per-skill verdicts | `handsfree` protocol authored, zero trials; `concierge` **executed** (MIXED); `hotseat` **executed** (PROVEN LIFT, structural); `spelunk` protocol + fixture/grader support authored, zero trials (zero-spend) |

...
| `outcomes/` | baseline-vs-skill outcome protocols + per-skill verdicts | `handsfree` protocol authored (deferred); **executed**: `concierge` (MIXED), `hotseat` (PROVEN LIFT), `proof` (MIXED), `recall` (MIXED), `spelunk` (MIXED), `scout` (MIXED, adjudicated), `distill` (NO LIFT), `masterplan` (NO LIFT), `pilot` (NO LIFT) — all zero Codex; remaining 19 skills protocol-authored, trials in flight |

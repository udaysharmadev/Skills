...
| `outcomes/` | baseline-vs-skill outcome protocols + per-skill verdicts | `handsfree` protocol authored; `concierge` **executed** (MIXED); `hotseat` **executed** (PROVEN LIFT); `proof` **executed** (MIXED, opencode); `spelunk` + `scout` + `distill` + `masterplan` + `pilot` + `backend` + `blueprint` + `headroom` + `polish` + `friction` + `ditto` + `roadtest` + `sleuth` + `referee` + `hotpath` + `harden` + `unslop` + `janitor` + `frontpage` + `findable` + `frugal` + `cleared` + `runway` + `recall` protocols + graders authored, zero trials (zero-spend, no Codex) — all 28 skills covered |

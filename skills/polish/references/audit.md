...
- **Typography**: use the smallest coherent family and type scale supported by
  the product; evaluate line length with real content and viewports.
- **Color**: role-based palette; verify the applicable WCAG 2.2 contrast
  criterion and its exceptions; color is not the only signal.
...
- **Loading**: use measured latency and user expectations to choose progress
  treatment; never a frozen UI; optimistic
...
- **Errors**: human-readable, actionable, placed near the field/area
  ...
  in the app.
- **Motion**: purposeful (entrance, feedback, transition), tuned to the
  interaction, and respects `prefers-reduced-motion`. Tell: everything
  bounces because it can.

## Reach

- **Responsive**: real breakpoints by content, not three magic widths;
  no avoidable horizontal scroll on mobile; target sizes follow applicable
  accessibility requirements and product context; tables get a
